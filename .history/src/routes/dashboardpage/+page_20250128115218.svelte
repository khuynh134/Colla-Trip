<script lang="ts">
    ...
    function toggleSidebar() {
      ...
      sidebarWidth = sidebarExtended ? '200px' : '64px';
    }
    </script>
    ...
    <div class="min-h-screen bg-gray-50">
      <!-- Left Sidebar -->
      <nav 
        class="fixed left-0 top-0 h-full bg-[#3598db] text-white p-10 flex flex-col {sidebarTransition}"
        style="width: {sidebarWidth};"
        on:mouseenter={toggleSidebar}
        on:mouseleave={toggleSidebar}
      >
        <div class="flex items-center mb-20 mt-16">
          <img src="/Colla-TripLogo-rem.png" alt="Logo" class="w-20 h-10">
          {#if sidebarExtended}
            <span class="text-xl font-bold ml-4">Colla-Trip</span>
          {/if}
        </div>
        ...
        <div class="space-y-4 mt-8">
          ...
          <!-- Add the rest of the sidebar items -->
        </div>
      </nav>
      ...
      >
        <div class="px-4 sm:px-8">
          ...
          <div class="flex flex-col sm:flex-row justify-between items-center mb-8 sm:mb-12 pt-4 sm:pt-8">
            ...
            <button class="bg-[#3598db] text-white px-8 sm:px-12 py-3 rounded-lg text-lg">
              Create Trip
            </button>
          </div>
