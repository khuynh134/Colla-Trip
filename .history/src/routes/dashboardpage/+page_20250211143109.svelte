<script lang="ts">
  ...
  // Modify your existing handleCreateTripClick function
function handleCreateTripClick(event: Event) {
    event.preventDefault();
    createFormOpen = true;
}

// Add function to close form
function closeCreateForm() {
    createFormOpen = false;
}

// Add state for managing invited members
...
function updateMember(index: number, value: string) {
    ...
    invitedMembers = invitedMembers; // trigger Svelte reactivity
}
...
<div class="fixed right-0 top-0 h-full w-96 bg-white shadow-lg transform transition-transform duration-300 ease-in-out {createFormOpen ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto">
  <div class="p-6">
      <!-- Header -->
...
<!-- Optional: Add an overlay when form is open -->
{#if createFormOpen}
  <div 
