...
export async function POST({ request }) {
    try{
        const { name, description, activity_date, trip_id } = await request.json(); //parse JSON body 
        //Validate date format
        if( !/^\d{4}-\d{2}-\d{2}$/.test(activity_date)){
            return json({ error: 'Invalid date format. Use YYYY-MM-DD.', status: 400 });
        }
        //validate input
        if (!name || !description || !activity_date || !trip_id) {
            return json({error: 'Name, description, date, and trip ID are required', status: 400});
            
        }

        const [activity] = await sql`
            INSERT INTO activities (name, description, votes, activity_date, trip_id)
            VALUES (${name}, ${description}, 0, ${activity_date}::date, ${trip_id} )
            RETURNING id, name, description, votes, activity_date, trip_id
            `;
