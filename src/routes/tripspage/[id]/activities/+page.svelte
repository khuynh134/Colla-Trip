...
function formatDisplayDate(dateString: string | Date | null) {
    ...
    async function clearResults() {
        try {
            ...
            if (response.ok) {
                voteResults = [];
                activities = activities.map(a => ({...a, votes: 0}));
                console.log('Votes cleared for this trip');
            } else {
                const errorData = await response.json();
                console.error('Failed to clear polling results');
                alert(`Error: ${errorData.error}`);
            }
        } catch (error) {
            console.error('Error clearing polling results: ', error);
